...
/**
 * Match strings in the format 'A, N%, N%, a' where A, N, a are numbers.
 *
 * ✓ '322, 79%, 52%, 1', '322 , 79%, 52%, 0.4', '322, 79%,52 %, 1', '1000, 79%, 52%, 1', '1000, 79%, 52%, 100'
 * ✗ '-1, 0, 0', '322, 79, 52%, -2'
 */
const HSLA_SHORT = /^(([0-9]+)(\s)*,(\s)*([0-9]+%)(\s)*,(\s)*([0-9]+%)(\s)*,(\s)*(0(\.\d+)?|1(\.0+)?))$/i

/**
 * Match strings in the format 'hsl(A, N%, N%)' where A, N are numbers.
 *
 * ✓ 'hsla(322, 79%, 52%, 1)', 'hsla(322 , 79%, 52%, 0.4)', 'hsla(322, 79%,52 %, 1)', 'hsla(1000, 79%, 52%, 100)'
 * ✗ '-1, 0, 0, 0', 'hsla(322, 79, 52%, 1)'
 */
const HSLA_LONG = /^((hsla(\s)*\()(\s)*([0-9]+)(\s)*,(\s)*([0-9]+%)(\s)*,(\s)*([0-9]+%),(\s)*(0(\.\d+)?|1(\.0+)?)(\s)*(\)))$/i
